...
ObservationState = TypedDict(
    "ObservationState",
    {
        ...
        # F/T sensor data added by Dylan Kavanaugh 4.20.2026
...
class AICRos2Interface:
    ...
    wrist_wrench_sub: Subscription[WrenchStamped]
    ...
    def connect(
        ...
        wrist_wrench_cb: Callable[[WrenchStamped], None], # F/T sensor data added by Dylan Kavanaugh 4.20.2026
    ) -> "AICRos2Interface":
        if not rclpy.ok():
            rclpy.init()
        ...
        # subscribe to the wrist_wrench stamped # F/T sensor data added by Dylan Kavanaugh 4.20.2026
        wrist_wrench_sub = node.create_subscription(
            WrenchStamped,
        ...
        return AICRos2Interface(
            ...
            # F/T sensor data added by Dylan Kavanaugh 4.20.2026
            wrist_wrench_sub=wrist_wrench_sub,
            logger=logger,
        )
...
class AICRobotAICController(Robot):
    ...
    def __init__(self, config: AICRobotAICControllerConfig):
        ...
        self.last_wrist_wrench: WrenchStamped | None = None
    ...
    def connect(self, calibrate: bool = True) -> None:
        ...
        # F/T sensor data added by Dylan Kavanaugh 4.20.2026
        def wrist_wrench_cb(msg: WrenchStamped):
            self.last_wrist_wrench = msg

        self.ros2_interface = AICRos2Interface.connect(
            controller_state_cb, joint_states_cb, wrist_wrench_cb # F/T sensor data added by Dylan Kavanaugh 4.20.2026
        )
    ...
    def get_observation(self) -> dict[str, Any]:
        ...
        if not self.last_controller_state or not self.last_joint_states \
            or not self.last_wrist_wrench: # F/T sensor data added by Dylan Kavanaugh 4.20.2026
            return {}
        ...
        wrist_wrench = self.last_wrist_wrench.wrench
        controller_state_obs: ObservationState = {
            ...
            # F/T sensor data added by Dylan Kavanaugh 4.20.2026
            "wrist_wrench.force.x": wrist_wrench.force.x,
            "wrist_wrench.force.y": wrist_wrench.force.y,
            "wrist_wrench.force.z": wrist_wrench.force.z,
            "wrist_wrench.torque.x": wrist_wrench.torque.x,
            "wrist_wrench.torque.y": wrist_wrench.torque.y,
            "wrist_wrench.torque.z": wrist_wrench.torque.z,
        }
