...
'''
The path for this file is located at:

~/ws_aic/src/aic/aic_utils/lerobot_robot_aic/lerobot_robot_aic/aic_teleop.py
'''
...
# implementation of the shadow teleop

@TeleoperatorConfig.register_subclass("aic_shadow_teleop")
@dataclass(kw_only=True)
class AICShadowTeleopConfig(TeleoperatorConfig):
    pass


class AICShadowTeleop(Teleoperator):
    '''
    Shadow teleop is a teleop that is used to control the robot from a remote location.
    Passive recorder - captures the actions from /aic_controller/pose_commands.
    '''
    config_type = AICShadowTeleopConfig

    def __init__(self, config: AICShadowTeleopConfig):
        super().__init__(config)
        self._is_connected = False
    ...
    def connect(self, calibrate: bool = True) -> None:
        '''
        Connect to the shadow teleop node
        which is a passive recorder of the actions
        from the /aic_controller/pose_commands topic1
        '''
        if not rclpy.ok():
            rclpy.init()

        self._node = rclpy.create_node("shadow_teleop")
        # subscribe to the pose commands
        ...
        self._executor = SingleThreadedExecutor()
        self._executor.add_node(self._node)
        Thread(target=self._executor.spin, daemon=True).start()

        self._is_connected = True

    def _cb(self, msg: MotionUpdate) -> None:
        self._last_action = {
            "linear.x": msg.velocity.linear.x,
            "linear.y": msg.velocity.linear.y,
            "linear.z": msg.velocity.linear.z,
            "angular.x": msg.velocity.angular.x,
            "angular.y": msg.velocity.angular.y,
            "angular.z": msg.velocity.angular.z,
        }
